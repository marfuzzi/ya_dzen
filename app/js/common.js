...
function largeCardTemplate(card) {
    ...
    <div class="card card_${size}">
        ...
        <div class="card__content_${size}">
            ${getTitle(size, titleColor, title)}
            ${getDescription(size, description)}
            ${getActions(size)}
        </div>
    </div>
    `;
...
function mediumCardTemplate(card) {
    ...
    return `
        <div class="card card_${size}">
            ${getTitle(size, titleColor, title)}
            ${getTagPicture(image, size, title)}
            <div class="card__content_${size}">
                ${getDescription(size, description)}
                ${getActions(size)}
            </div>
...
const cards = data.map(card => {
    switch (card.size) {
        case's':
            return smallCardTemplate(card);
        case'm':
            return mediumCardTemplate(card);
        case'l':
            return largeCardTemplate(card);
        default:
            return smallCardTemplate(card);
    }
});
...
content.insertAdjacentHTML('afterbegin', cards.join(''));
